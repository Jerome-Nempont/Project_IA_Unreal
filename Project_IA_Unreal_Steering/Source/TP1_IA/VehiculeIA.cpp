...
{
	FVector VectorDist = TargetLocation - GetActorLocation();
	float dist = VectorDist.Size();
	float RampedSpeed = MaxSpeed * (dist / SlowingDistance);
	float ClippedSpeed = FMath::Min(RampedSpeed, MaxSpeed);
	FVector VelocityDesired = VectorDist * (ClippedSpeed / dist);
	return VelocityDesired - Velocity;
}
...
FVector AVehiculeIA::PursuitVelocity()
{
	...
	float T = dist / Velocity.Size() * TurningParameter;
